...
vk.updates.on('message', async (context) => {
  ...
  if(context.messagePayload?.command == 'checkMemes') {
    ...
    const action = await context.question(`Мем #${memId + 1}`, {
      attachment: memes[memId],
      keyboard: likeAndDis(memId)
    });

    if(action.messagePayload.command == 'likeAndDis') {
      if(action.messagePayload.action == 'like') {
        users[context.senderId].likes += Number(1);
        return context.send({
          message: `Вы поставили лайк мему #${action.messagePayload.memId + 1}`
        });
      }; 
      if(action.messagePayload.action == 'dis') {
        users[context.senderId].dislikes += Number(1);
        return context.send({
          message: `Вы поставили дизлайк мему #${action.messagePayload.memId + 1}`
        });
      };
    };
  };
